...
int main() {
    ...
    std::cout << "\nTEST PHASE 3\n";
    /*if (p6.isRoot(2)) {
        std::cout << "two is a root of " << p6 << '\n';
    }
    else {
        std::cout << "two is a not root of " << p6 << '\n';
    }*/
    ...
    /*
	//p1[2] = 4.4; //this should not compile <-- test. Since it is const.
    */
    ...
    //p3 = p3 + 2;
    Logarithm l1{p3 + 2, 2.0, 3.0, 10}; //Does not work with constructor since missing + operator atm
    ...
    //*e1 = *e2; //<-- should not compile!! Blir röd där.
    ...
    Polynomial p6{p4};
    Polynomial p7{p2};
    //Logarithm p6{ l3 }; Eftersom vi har clone i Expression funkar detta inte
...
//Vrf använder man inte bara en copy constructor vrf behövs clone()? 
//Förstår ej frågan, är det i expressions vrf copy existerar eller 
//är det en fråga om vrf vi ens har clone() i subbarna och inte bara använder copy.
//Vet ej om man clonar, de kollar om expression E är en logarithm eller ett polynom

//När vi copierar ett polynom är det inget problem att köra en copy konstructor i polynom men när vi kopierar en logaritm
//så har vi även en expression som är okänd typ. Därför måste FUNKTIONEN COPY DECLARERAS INUTI EXPRESSION OCH SEDAN LÅTA COMPILATORN SJÄLV
//välja funktion beroende av typ.



//const Polynomial pp{3, v1}; pp[2] = 3.3;
//Funkar ej då det är en const, alltså man får inte ändra på värderna i pp.